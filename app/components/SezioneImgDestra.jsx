...
export default function SezioneInterna({
    ...
    return (
        <div className={`h-[100vh] ${backgroundColor} bg-opacity-85 ${textColor} w-full flex  ${ordineImgText} justify-center gap-12 md:gap-0 items-center`}>
            ...
            <div className={`w-[90%] py-2 relative lg:w-[100%]  mx-[0px] flex lg:items-start ${ordineTitoloTesto} gap-2 md:gap-0 z-10`}>
                {showTitle && (
                    <h2 className={`fontPrimary mx-1 ${direzioneTitolo} px-[10px] ${titleColor} uppercase text-xl lg:text-2xl mb-4 z-20 md:ml-[50%]`}>{title}</h2>  
                )}
                <h3 className={`fontPrimary  ${direzioneTesto} px-[10px] text-sm lg:text-xl ml-[2%] md:ml-[50%] mr-0 2xl:mr-80`}>
                    {text}
                </h3>
                ...
                {showDettaglio && (
                    <div className={`absolute top-0 left-0 w-full h-[40px] ${dettaglioColor}`}></div>
                )}
            </div>
